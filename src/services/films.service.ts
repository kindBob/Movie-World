...
export class FilmsService {
  ...
  getFilms(
    ...
    return this.http
      ...
      .pipe(catchError((error) => this.usersService.processError(error)));
  }

  getFilm(id: number): Observable<Film> {
    return this.http
      .get<Film>(this.url + 'films/' + id, this.getTokenHeader())
      .pipe(catchError((error) => this.usersService.processError(error)));
  }

  createFilm(film: Film): Observable<Film> {
    return this.http
      .post<Film>(this.url + 'films', film, this.getTokenHeader())
      .pipe(catchError((error) => this.usersService.processError(error)));
  }

  updateFilm(film: Film): Observable<Film> {
    return this.http
      .put<Film>(this.url + 'films/' + film.id, film, this.getTokenHeader())
      .pipe(catchError((error) => this.usersService.processError(error)));
  }
}
