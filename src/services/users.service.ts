...
export class UsersService {
  ...
  users: User[] = [
    new User('JankoService', 'janko@janko.sk'),
  ...
  isLoggedInAsync(): Observable<boolean> {
    ...
    return this.http.get<void>(this.url + 'check-token/' + this.token).pipe(
      map(() => true),
      catchError((error) =>
        this.logout().pipe(switchMap(() => this.processError(error)))
      ),
  ...
  deleteUser(id: number): Observable<boolean> {
    return this.http
      .delete<void>(this.url + 'user/' + id + '/' + this.token)
      .pipe(
        map(() => true),
        catchError((error) => this.processError(error))
      );
  ...
  deleteGroup(id: number): Observable<boolean> {
    return this.http
      .delete<void>(this.url + 'group/' + id + '/' + this.token)
      .pipe(
        map(() => true),
        catchError((error) => this.processError(error))
      );
